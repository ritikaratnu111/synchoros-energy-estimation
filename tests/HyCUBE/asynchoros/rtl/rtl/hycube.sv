...
if (address[12]==1'b0 && address[13]==1'b0 && scan_data_or_addr == 1'b0) begin
...
if (reset)
	...
	if (start_exec) begin
        if ((addr_shifted >= loop_end_dash) || (addr_shifted < loop_start))
...
//changed from 10:2 to 9:2 to write to same memory addresses
assign addr_dm_shifted0 = (!start_exec) ? address[9:2] : addr_dm_out[0][0];
...
assign addr_dm_shifted4 = (!start_exec) ? address[9:2] : addr_dm_out[1][0];
...
assign addr_dm_shifted8 = (!start_exec) ? address[9:2] : addr_dm_out[2][0];
...
assign addr_dm_shifted12 = (!start_exec) ? address[9:2] : addr_dm_out[3][0];
